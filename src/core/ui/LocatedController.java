...
public class LocatedController {
        ...
        for (CheckBox checkBox : checkBoxes){
            if (checkBox.isSelected()) {
                if (!checkLocaled(checkBox.getText()))
                    InitForm.locateds.add(new Located(checkBox.getText()));
            } else {
                InitForm.locateds.remove(new Located(checkBox.getText()));
            }
        }
    }

    private boolean checkLocaled(String search){
        for(Located str: InitForm.locateds) {
    ...
    private void initCheckBoxLocation(){
        ...
        if (InitForm.locateds.size() > 0){
            for (Located location : InitForm.locateds) {
                for (CheckBox checkBox : checkBoxes) {
